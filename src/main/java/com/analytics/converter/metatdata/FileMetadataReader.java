...
import com.analytics.converter.dao.MetadataError;

public class FileMetadataReader implements MetadataReader {

	private static final Logger LOG = Logger.getLogger(FileMetadataReader.class);
	...
	public List<Metadata> readMetadata(String path) throws MetadataException {
		...
		// If there is any error in metadata then add it to List<MetadataError> and continue processing.
		for (CSVRecord record : records) {
			try {
				...
				metadata = new Metadata(record.get(Headers.NAME).trim(), Integer.parseInt(record.get(Headers.LENGTH).trim()),
	...
	private String getLine(CSVRecord record) {
		return record.get(0) + FileConverterConstants.METADATA_COLUMN_DELIMITER +
				record.get(1) + FileConverterConstants.METADATA_COLUMN_DELIMITER +
				record.get(2);
	...
	private MetadataError validateDataType(CSVRecord record) {
		...
		case FileConverterConstants.DATA_TYPE_DATE:
			break;
		case FileConverterConstants.DATA_TYPE_STRING:
			break;
		case FileConverterConstants.DATA_TYPE_NUMERIC:
			break;
		default:
			mdError = new MetadataError(record.getRecordNumber(), record.toString(),
